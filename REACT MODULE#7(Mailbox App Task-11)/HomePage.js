  ...
  async function emailLinkClickHandler(mail){
    ...
    Object.entries(data).forEach(async(entity)=>{
      if(entity[1].emailBody===mail.emailBody){
        const response= await fetch(`https://mailbox-app-1e6f1-default-rtdb.firebaseio.com/to-${mail.toEmail.replace(/[@.]/g,'')}/${entity[0]}.json`,{
          method:'PUT',
                  ...
                  <ul style={{ display: 'flex',paddingLeft:'10px',paddingRight:'5px'}}>
                    ...
                    {mail.status==='read'?
                      <Col xs={2} className={classes.binIcon}>
                        ...
                        <img src='/bin_Icon.jpeg' onClick={(e)=>deletemailHandler(e,mail)} alt='Bin Icon'/>
                      </Col>:
