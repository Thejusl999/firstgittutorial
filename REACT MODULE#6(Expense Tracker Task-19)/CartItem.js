...
const CartItem = (props) => {
  ...
  const increaseHandler=(title,quan)=>{
    let ind;
  ...
  const decreaseHandler=(title,quan)=>{
    let ind;
    cartItems.map((item,index)=>{
      if(item.title===title){
        ind=index;
        ...
        <div key={Math.random()}>{item.quantity>0&&<li className={classes.item}>
          ...
          <div className={classes.details}>
            ...
            <div className={classes.actions}>
              <button onClick={()=>decreaseHandler(item.title,item.quantity)}>-</button>
              <button onClick={()=>increaseHandler(item.title,item.quantity)}>+</button>
            </div>
          </div>
      </li>}</div>))}
    </>
